...
import { testFileSystem } from "@/lib/testFileSystem";
...
export function Browser() {
  ...
  // Run file system tests when browser component mounts
  useEffect(() => {
  ...
  const navigate = useCallback(
    (tabId: number, url: string, addToHistory = true) => {
      ...
      if (addToHistory) {
        setHistory((prev) => {
          const tabHistory = [
            ...(prev[tabId] || []).slice(0, (historyIndex[tabId] || 0) + 1),
          ];
      ...
      <div className="flex flex-col border-b">
        <div className="flex items-center gap-2 p-2">
          <Button
            ...
            onClick={goBack}
            disabled={
              !history[activeTab] || (historyIndex[activeTab] || 0) === 0
            }
          >
          ...
          <Button
            ...
            disabled={
              !history[activeTab] ||
              (historyIndex[activeTab] || 0) >= (history[activeTab]?.length || 0) - 1
            }
          >
